        ...
        switch(instr.op) {
            ...
            case FUNC_ENTRY:
             ...
             case NEW:
                 ...
                 out.println("   new " + cls);
                 out.println("   dup");
                 String[] newParts = instr.arg2.split(":");
                 int argc = Integer.parseInt(newParts[0]);
                 String ctorSig = newParts.length > 1 ? newParts[1] : null;

                 if (ctorSig == null) {
                     // Fallback
                     StringBuilder sb = new StringBuilder("(");
                     for(int i=0; i<argc; i++) sb.append("I");
                     sb.append(")V");
                     ctorSig = sb.toString();
                 }

                 out.println("   invokespecial " + cls + "/<init>" + ctorSig);
             ...
             case CALL_VIRTUAL:
                 String mNameRaw = instr.arg2;
                 int args = 0;
                 String callSigStr = null;

                 if (mNameRaw.contains(":")) {
                     String[] p = mNameRaw.split(":");
                     mNameRaw = p[0];
                     args = Integer.parseInt(p[1]);
                     if (p.length > 2) callSigStr = p[2];
                 }

                 String cName = "Main";
                 String meth = mNameRaw;
                 if (mNameRaw.contains(".")) {
                     String[] p = mNameRaw.split("\\.");
                     cName = p[0];
                     meth = p[1];
                 ...
                 if (callSigStr == null) {
                    // Fallback
                     ...
                     for(int i=0; i < args - 1; i++) callSig.append("I");
                     callSig.append(")I");
                     callSigStr = callSig.toString();
                 }

                 out.println("   invokevirtual " + cName + "/" + meth + callSigStr);
                 // If return type is void, we might need to handle it?
                 // But CALL_VIRTUAL result is stored in target.
                 // If void, target is likely unused or we should not store?
                 // TAC expects result.
                 // However, generated bytecode for void method does not put anything on stack.
                 // But TAC treats it as expression?
                 // If callSigStr ends with V, we should NOT store.
                 if (!callSigStr.endsWith(")V")) {
                     storeVar(out, instr.target, callSigStr.endsWith(";") || callSigStr.startsWith("L") || callSigStr.startsWith("["));
                 }
                 break;
